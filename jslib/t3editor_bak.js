...
var t3editor = function()	{
  ...
  function t3editor(theTextarea, index) {
    ...
    this.fitem_options_overlay.innerHTML = '<ul>'+
				...
				'<li style="color:grey"><input type="checkbox" disabled="disabled" /> Snippets</li>'+
  ...
  t3editor.prototype = {
	...
	footeritem_demo_click: function() {
		...
		insertTextAtCursor(this.win, "page = PAGEpage.10 = TEXT	page.10 {\n	value = lala\n}\n");
    ...
    updateLinenum: function()      {
            var bodyContentLineCount = this.container.innerHTML.match(/<br/gi).length;
	...
	saveAjaxOnSuccess: function(ajaxrequest) {
		...
		if (ajaxrequest.status == 200
			...
			alert("An error occured while saving the data. Try to disable the advanced editor and save again.");
		};
		this.modalOverlay.hide();
	},
